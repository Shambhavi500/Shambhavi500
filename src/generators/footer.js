...
export function generateFooterSvg(data) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="100%" height="${height}" fill="none">
    ...
    <rect width="${width}" height="${height}" rx="${theme.radius.card}" fill="url(#foot_radialAura)" />
    ...
    ${renderAbstractSilhouette(width / 2 - 45, 10, 0.45, 0.08, theme.colors.barbiePink)}

    <!-- Centerpiece Closing Statement -->
    <g transform="translate(${width / 2}, 66)">
      <text x="0" y="0" text-anchor="middle" class="display-title" font-size="20" font-weight="800" fill="${theme.colors.deepCharcoal}">
        &quot;Crafting intelligent systems with elegance and precision.&quot;
      </text>
      <text x="0" y="24" text-anchor="middle" class="code-mono" font-size="9.5" font-weight="700" fill="${theme.colors.retroMagenta}" letter-spacing="0.16em">
    ...
    <path d="M ${width / 2 - 120} 104 C ${width / 2 - 40} 100, ${width / 2 + 40} 108, ${width / 2 + 120} 104"
          stroke="url(#foot_barbieGrad)" stroke-width="2" stroke-linecap="round" opacity="0.8" />
    ...
    <g transform="translate(${width - 42}, 136)">
      <text x="0" y="0" text-anchor="end" class="code-mono" font-size="9" font-weight="600" fill="${theme.colors.barbiePink}" letter-spacing="0.1em">
        DESIGNED WITH INTENTION · BUILT WITH CODE
      </text>
    </g>

    <!-- Restrained Luxury Glint Sparkles -->
    ${renderSparkle(width / 2 - 210, 62, 11, theme.colors.barbiePink)}
    ${renderSparkle(width / 2 + 210, 62, 11, theme.colors.dreamhouseBlush)}
    ${renderSparkle(width - 55, 42, 9, theme.colors.barbiePink)}
  </svg>`;
}
