import { escapeXml } from './theme.js';

export function generateReadme(data) {
  const username = data.username || 'Shambhavi500';
  const name = data.name || 'Shambhavi Patil';

  return `<div align="center">

<!-- HERO SECTION // ATELIER SHAMBHAVI -->
<a href="https://github.com/${username}">
  <img src="./assets/hero.svg" alt="${name} - Luxury Barbie-Inspired Developer Profile" width="100%" />
</a>

<br/>

<!-- HAUTE COUTURE RIBBON DIVIDER -->
<img src="./assets/divider.svg" alt="Divider" width="100%" />

<br/>

<!-- WHO IS SHAMBHAVI? // IDENTITY DOSSIER -->
<a href="https://github.com/${username}">
  <img src="./assets/identity.svg" alt="Identity Dossier - Shambhavi Patil" width="100%" />
</a>

<br/>

<!-- HAUTE COUTURE RIBBON DIVIDER -->
<img src="./assets/divider.svg" alt="Divider" width="100%" />

<br/>

<!-- DREAMHOUSE DASHBOARD // METRICS -->
<a href="https://github.com/${username}?tab=repositories">
  <img src="./assets/dashboard.svg" alt="Dreamhouse Dashboard - Telemetry &amp; Metrics" width="100%" />
</a>

<br/>

<!-- HAUTE COUTURE RIBBON DIVIDER -->
<img src="./assets/divider.svg" alt="Divider" width="100%" />

<br/>

<!-- DREAM PROJECTS // THE ATELIER COLLECTION -->
<a href="https://github.com/${username}?tab=repositories">
  <img src="./assets/projects.svg" alt="Dream Projects - Featured Open Source Systems" width="100%" />
</a>

</div>

### ✦ Atelier Project Directory

| Project | Domain / Category | Primary Architecture | Description |
| :--- | :--- | :--- | :--- |
| **[Ovio](https://github.com/${username}/Ovio)** | \`AI / MULTI-AGENT\` | **Python** · Multi-Agent Orchestration | AI-powered DaVinci Resolve video editing assistant and multi-agent workflow engine |
| **[Aira](https://github.com/${username}/Aira)** | \`FINTECH / AUTONOMOUS\` | **TypeScript** · Distributed Systems | Autonomous Revenue Recovery Operating System for the Indian Fintech Ecosystem |
| **[KrishiSahAI](https://github.com/${username}/KrishiSahAI)** | \`AGRITECH / AI ADVISORY\` | **Python** · Machine Learning | State-of-the-art AI advisory bringing precision agriculture to Indian farmhouses |
| **[KrishiSetu](https://github.com/${username}/KrishiSetu)** | \`BLOCKCHAIN / SUPPLY CHAIN\` | **TypeScript** · Web3 | Decentralized agri-supply chain solution ensuring transparent and fair farmer pricing |
| **[AlphaTrader-RL](https://github.com/${username}/AlphaTrader-RL)** | \`QUANT / REINFORCEMENT LEARNING\` | **Python** · RL Trading Engine | Reinforcement learning algorithms for automated trading execution |
| **[StockItUp](https://github.com/${username}/StockItUp)** | \`FINTECH / REAL-TIME\` | **JavaScript** · Live APIs | Real-time market tracking platform with interactive financial charting |
| **[NDVI_satellite](https://github.com/${username}/NDVI_satellite)** | \`EARTH OBSERVATION / GIS\` | **JavaScript** · Spectral Bands | Normalized Difference Vegetation Index analysis using multi-spectral satellite imagery |
| **[EPeekPahani](https://github.com/${username}/EPeekPahani)** | \`MOBILE / ANDROID\` | **Kotlin** · Android SDK | Digital crop survey and agricultural ground-truth verification application |

<div align="center">

<br/>

<!-- HAUTE COUTURE RIBBON DIVIDER -->
<img src="./assets/divider.svg" alt="Divider" width="100%" />

<br/>

<!-- TECH WARDROBE // THE ENGINEERING ENSEMBLE -->
<a href="https://github.com/${username}?tab=repositories">
  <img src="./assets/tech-wardrobe.svg" alt="Tech Wardrobe - Curated Technology Stack" width="100%" />
</a>

<br/>

<!-- HAUTE COUTURE RIBBON DIVIDER -->
<img src="./assets/divider.svg" alt="Divider" width="100%" />

<br/>

<!-- DEVELOPER RUNWAY // CODE CADENCE -->
<a href="https://github.com/${username}">
  <img src="./assets/runway.svg" alt="Developer Runway - Contribution Activity" width="100%" />
</a>

<br/>

<!-- HAUTE COUTURE RIBBON DIVIDER -->
<img src="./assets/divider.svg" alt="Divider" width="100%" />

<br/>

<!-- CAREER EDITION // ATELIER MILESTONES -->
<a href="https://github.com/${username}?tab=repositories">
  <img src="./assets/achievements.svg" alt="Career Edition - Milestone Badges" width="100%" />
</a>

<br/>

<!-- HAUTE COUTURE RIBBON DIVIDER -->
<img src="./assets/divider.svg" alt="Divider" width="100%" />

<br/>

<!-- FOOTER // DREAMHOUSE CLOSING -->
<a href="mailto:shambhavipatil5631@gmail.com">
  <img src="./assets/footer.svg" alt="Welcome to Shambhavi's Dreamhouse" width="100%" />
</a>

<br/><br/>

[![GitHub](https://img.shields.io/badge/GitHub-Shambhavi500-E0218A?style=for-the-badge&logo=github&logoColor=white)](https://github.com/${username})
[![Email](https://img.shields.io/badge/Email-shambhavipatil5631%40gmail.com-151122?style=for-the-badge&logo=gmail&logoColor=FF2D87)](mailto:shambhavipatil5631@gmail.com)
[![Status](https://img.shields.io/badge/Status-Building_Autonomous_Systems-E0218A?style=for-the-badge)](https://github.com/${username})

<br/><br/>

<sub>Curated with intention &amp; precision in the <b>Atelier No. 500</b>. Inspired by Barbie Couture &amp; Modern Systems Engineering.</sub>

</div>
`;
}
