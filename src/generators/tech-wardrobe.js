...
export function generateTechWardrobeSvg(data) {
  ...
  const columnsSvg = categories.map((cat, idx) => {
    ...
    const itemsSvg = cat.items.map((item, itemIdx) => {
      ...
      return `
        ...
        <g transform="translate(14, ${itemY})">
          ...
          <circle cx="12" cy="13" r="2.5" fill="${theme.colors.barbiePink}" />
          <text x="22" y="16.5" class="code-mono" font-size="8.8" font-weight="700" fill="${theme.colors.deepCharcoal}">
            ${escapeXml(item)}
          </text>
        </g>
      `;
    ...
    return `
      ...
      <g transform="translate(${x}, ${startY})">
        <rect x="0" y="0" width="${colWidth}" height="${colHeight}" rx="20"
              fill="${theme.colors.surface}" stroke="${theme.colors.border}" stroke-width="0.8" />

        <!-- Column Top Trim -->
        <rect x="0" y="0" width="${colWidth}" height="3.5" rx="1.75" fill="url(#ward_barbieGrad)" />

        <g transform="translate(14, 28)">
          <text x="0" y="0" class="code-mono" font-size="9" font-weight="700" fill="${theme.colors.retroMagenta}" letter-spacing="0.1em">
            ${escapeXml(cat.title)}
          </text>
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="100%" height="${height}" fill="none">
    ...
    <rect width="${width}" height="${height}" rx="${theme.radius.card}" fill="url(#ward_radialAura)" />
